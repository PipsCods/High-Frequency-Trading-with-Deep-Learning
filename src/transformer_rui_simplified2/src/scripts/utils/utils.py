...
def assign_risk_class_by_cumulative_std(
    ...
    Returns:
    - DataFrame enriched with ['risk_std', 'risk_quantile']
    """
...
class ReadyToTransformerDataset(Dataset):
    def __init__(self, df, categorical_variables, continuous_variables, seq_len, target_return, target_regime):
        ...
        target_tensor = np.full((num_times, num_symbols, 2), np.nan, dtype=np.float32)  # 2 targets per symbol
        ...
        for _, row in tmp.iterrows():
            ...
            input_tensor[t_idx, s_idx, :] = np.array(all_vals, dtype=np.float32)
            target_tensor[t_idx, s_idx, 0] = float(row[target_return])
            target_tensor[t_idx, s_idx, 1] = float(int(row[target_regime]))

        for t in range(seq_len, num_times - 1):
            X_seq = input_tensor[t - seq_len:t, :, :]  # [seq_len, num_symbols, num_features]
            Y_targets = target_tensor[t, :, :]         # [num_symbols, 2]

            if not np.isnan(X_seq).any() and not np.isnan(Y_targets).any():
                self.samples.append((X_seq, Y_targets))
